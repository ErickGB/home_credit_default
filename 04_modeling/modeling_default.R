...
PATH_IN <- "./00_Data/"
PATH_OUT <- "./00_Data/output/"

#---------------------------
cat("Loading data...\n")

bbalance <- read_csv("./00_data/bureau_balance.csv") %>% janitor::clean_names()
bureau <- read_csv("./00_data/bureau.csv") %>% janitor::clean_names()
cc_balance <- read_csv("./00_data/credit_card_balance.csv") %>% janitor::clean_names()
payments <- read_csv("./00_data/installments_payments.csv") %>% janitor::clean_names()
pc_balance <- read_csv("./00_data/POS_CASH_balance.csv") %>% janitor::clean_names()
prev <- read_csv("./00_data/previous_application.csv") %>% janitor::clean_names()
tr <- read_csv("./00_data/application_train.csv") %>% janitor::clean_names()
te <- read_csv("./00_data/application_test.csv") %>% janitor::clean_names()
...
sum_bbalance <- bbalance %>%
  ...
  summarise(across(starts_with("months_balance"), 
...
sum_bureau <- bureau %>% 
  ...
  select(-c(sk_id_bureau, credit_type)) %>% 
  group_by(sk_id_curr) %>% 
  summarise(across(days_credit:amt_annuity, 
                   list(mean = mean, sd = sd, 
                        min = min, max = max, sum = sum,
                        n_distinct = n_distinct
                   )))
